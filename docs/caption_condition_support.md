...
- the checked-in inspection/conversion tooling also recognizes the public `Aratako/Irodori-TTS-500M-v3` schema intentionally, although runtime execution for the v3 duration-predictor path is still follow-up work
...
| `scripts/convert_weights.py` | supported for the inspected family | Detects base-v2 vs VoiceDesign for caption-conditioned checkpoints, validates family-specific tensor/config layouts, exports caption-conditioned `.npz` archives, and also recognizes the public `Aratako/Irodori-TTS-500M-v3` schema for inspection/conversion-only support. |
...
### 4. V3 runtime support is not included in this support statement

The converter can now validate and export the public `Aratako/Irodori-TTS-500M-v3` checkpoint layout intentionally, but that does **not** mean the MLX runtime implements the duration predictor yet. End-to-end v3 generation remains follow-up work under issue #49.
