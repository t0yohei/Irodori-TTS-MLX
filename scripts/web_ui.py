...
from scripts import generate_wav
...
def build_generate_argv(config: WebGenerationConfig, *, output_wav: str, metadata_json: str) -> list[str]:
    ...
    _add_optional(argv, "--codec-artifact-repo", config.codec_artifact_repo)
    ...
    argv.extend(
        [
            "--cfg-scale-text",
            str(float(config.cfg_scale_text)),
            "--cfg-scale-caption",
            str(float(config.cfg_scale_caption)),
            "--cfg-scale-speaker",
            str(float(config.cfg_scale_speaker)),
            "--cfg-guidance-mode",
            config.cfg_guidance_mode,
            "--cfg-min-t",
...
def run_generation(config: WebGenerationConfig) -> tuple[str | None, str, str]:
    ...
    rc = 1
    try:
        sys.argv = ["irodori-tts-generate", *argv]
        with contextlib.redirect_stdout(stdout), contextlib.redirect_stderr(stderr):
            try:
                rc = int(generate_wav.cli_main())
...
def build_ui() -> Any:
    ...
    with gr.Blocks(title="Irodori-TTS-MLX") as demo:
        ...
        with gr.Accordion("Codec", open=False):
            codec_runtime_mode = gr.Dropdown(["persistent", "subprocess", "mlx", "mlx-decode", "mlx-decode-subprocess"], value="persistent", label="Codec runtime mode")
            codec_artifact_repo = gr.Textbox(label="Codec artifact repo", placeholder=DEFAULT_CODEC_ARTIFACT_REPO)
        ...
        with gr.Accordion("Sampling", open=False):
            cfg_scale_text = gr.Number(label="CFG scale text", value=3.0)
            cfg_scale_caption = gr.Number(label="CFG scale caption", value=3.0)
            cfg_scale_speaker = gr.Number(label="CFG scale speaker", value=5.0)
            cfg_guidance_mode = gr.Dropdown(["independent", "joint", "reduced"], value="independent", label="CFG guidance mode")
...
def main(argv: list[str] | None = None) -> int:
    ...
    demo = build_ui()
    demo.launch(server_name=args.host, server_port=args.port, share=bool(args.share), inbrowser=bool(args.inbrowser))
    return 0
