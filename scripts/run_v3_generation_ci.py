#!/usr/bin/env python3
"""Run hosted Apple Silicon v3 end-to-end generation coverage.

This helper is intended for GitHub Actions on Apple Silicon runners. It:

1. downloads the public v3 checkpoint from Hugging Face
2. inspects and validates the checkpoint layout
3. converts the checkpoint into an MLX-friendly `.npz`
4. runs `scripts/generate_wav.py` against the converted weights without
   `--seconds` so the duration predictor path is exercised
...
DEFAULT_REPO_ID = "Aratako/Irodori-TTS-500M-v3"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Download the public v3 checkpoint, convert it, and run the full "
            "generate_wav.py path with predicted duration."
        )
    )
...
def _require_hf_hub_download():
    try:
        ...
        raise RuntimeError(
            "huggingface_hub is required for the hosted v3 generation helper. Install it before running this script."
        ) from exc
...
def _model_config_payload(config: dict[str, Any] | None) -> dict[str, Any]:
    ...
    if payload.get("use_duration_predictor") is not True:
        raise RuntimeError("Hosted v3 generation requires a duration-predictor checkpoint config.")
    if payload.get("use_caption_condition") is True:
        raise RuntimeError("Hosted v3 generation expects the non-caption v3 checkpoint family.")
    return payload
...
def run_generation(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="v3-generation-download-") as tmp_download, tempfile.TemporaryDirectory(
        prefix="v3-generation-output-"
    ) as tmp_output:
        ...
        report = convert_weights.build_report(
            checkpoint_path,
            local_output_dir / "irodori-v3.npz",
            records,
            validation,
            dry_run=False,
        )
        arrays = convert_weights.records_to_arrays(records, checkpoint_family=validation["checkpoint_family"])
        weights_path = local_output_dir / "irodori-v3.npz"
        convert_weights.write_npz_atomic(weights_path, arrays)

        model_config_payload = _model_config_payload(config)
        model_config_path = local_output_dir / "v3-model-config.json"
        model_config_path.write_text(
            json.dumps(model_config_payload, ensure_ascii=False, indent=2, sort_keys=True),
            encoding="utf-8",
        )

        output_wav = local_output_dir / "v3-hosted.wav"
        metadata_json = local_output_dir / "v3-generation-metadata.json"
        stdout_path = local_output_dir / "v3-generate.stdout.json"
        stderr_path = local_output_dir / "v3-generate.stderr.txt"
        ...
        if duration_mode != "predicted":
            raise RuntimeError(
                f"Hosted v3 generation expected duration_mode='predicted', got {duration_mode!r}."
            )
...
def main() -> int:
    ...
    if args.json_output:
        ...
        print(f"duration_mode: {result['generation']['result']['duration_mode']}")
