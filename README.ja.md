...
upstream で検証済みの低 step recipe が Sway Sampling を使う場合は、MLX runtime でも `--t-schedule-mode sway --sway-coeff -1.0` を指定できます。default は引き続き `--t-schedule-mode linear` です。timestep schedule を upstream と揃えることは recipe parity に有用ですが、MLX runtime は codec artifact や実行詳細が異なるため、これだけで音声が完全一致するわけではありません。

upstream Irodori-TTS で学習・検証した Speaker Inversion embedding は、`--ref-embed` で MLX inference に再利用できます。embedding は `.speaker.safetensors` で、shape `(speaker_dim)`、`(sequence, speaker_dim)`、または `(1, sequence, speaker_dim)` の speaker-state tensor を1つ含む必要があります。`--ref-embed` は `--reference-wav` / `--no-reference` と同時指定できません。DACVAE reference encode を迂回し、JSON metadata には `speaker_condition_source: "embedding"` が記録されます。
...
local v3 hosted-layout の直接 smoke path:
