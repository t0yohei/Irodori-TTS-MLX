...
upstream Irodori-TTS で学習・検証した Speaker Inversion embedding は、`--ref-embed` で MLX inference に再利用できます。embedding は `.speaker.safetensors` で、shape `(speaker_dim)`、`(sequence, speaker_dim)`、または `(1, sequence, speaker_dim)` の speaker-state tensor を1つ含む必要があります。`--ref-embed` は `--ref-wav` / `--no-ref` と同時指定できません。DACVAE reference encode を迂回し、JSON metadata には `speaker_condition_source: "embedding"` が記録されます。
