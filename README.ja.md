...
public runtime のデフォルトは承認済み hosted DACVAE codec artifact を使い、upstream `irodori_tts.codec.DACVAECodec`、`torch`、`torchaudio` を必要としません。upstream PyTorch DACVAE bridge は fallback、parity、upstream 比較用の明示的な `persistent` / `subprocess` codec mode として残します。
...
| PyTorch bridge-backed DACVAE codec path | Supported fallback | 明示的な `persistent` / `subprocess` mode は upstream `irodori_tts.codec.DACVAECodec` で codec encode/decode を行うため、upstream dependency の install または import 可能化が必要です。 |
| MLX DACVAE decode for no-reference generation | Supported | 承認済み codec artifact を使うと no-reference v3 / VoiceDesign の decode を PyTorch bridge から外せます。 |
...
upstream Irodori-TTS が必要なのは、明示的な PyTorch bridge / parity workflow だけです。その場合は同じ環境に入れるか、既存 checkout を import 可能にします。
...
# bridge/parity mode で未インストール checkout を使う場合:
# export PYTHONPATH=/path/to/Irodori-TTS:${PYTHONPATH:-}
```

Python 3.11 では、`runtime`、`bench`、`parity`、`dev` extra は同じ virtual environment
...
明示的な `persistent` / `subprocess` codec mode では upstream `irodori_tts.codec.DACVAECodec` が必要です。デフォルトの standalone MLX runtime path は承認済み hosted codec artifact を使います。再現性のある環境構築は [docs/packaging.md](docs/packaging.md) と [docs/upstream_dependency.md](docs/upstream_dependency.md) を参照してください。
...
デフォルトでは、これらの例は `--codec-runtime-mode mlx` と承認済み hosted DACVAE codec artifact を使います。local staged artifact を使う場合は `--codec-artifact-dir` または `--codec-path` を渡します。decode-only artifact 実験では `mlx-decode` を使えますが、`mlx-decode` で reference-audio encode が必要な場合は PyTorch bridge に fallback します:
...
no-reference の full-MLX run は metadata に `codec_decode_backend: "mlx"` と `codec_encode_backend: "not-required"` を報告します。`mlx` で reference-audio 生成する場合は、codec artifact 内の実行可能な Semantic-DACVAE encoder / decoder tensor を使い、`codec_encode_backend: "mlx"` と `codec_decode_backend: "mlx"` を報告します。`mlx-decode` で reference-audio 生成を行う場合だけ、従来どおり PyTorch encode fallback を使い、その backend を `codec_encode_backend` に記録します。承認済み hosted repo がない場合は、下の local conversion fallback を使います。詳細は [docs/hosted_weights_usage.md](docs/hosted_weights_usage.md) を参照してください。
...
- upstream import / `DACVAECodec`: 明示的な `persistent`、`subprocess`、または `mlx-decode` の reference-audio fallback path だけに関係します。active venv に upstream Irodori-TTS を install するか、`PYTHONPATH=/path/to/Irodori-TTS:${PYTHONPATH:-}` を設定します。
...
- hosted DACVAE codec: `irodori_dacvae_codec_manifest.json`、`dacvae-codec.npz`、`codec_metadata.json`、`checksums.sha256`、`license_review.status: "approved"` を確認し、だめなら local `--codec-path` / `--codec-artifact-dir` または明示的な `--codec-runtime-mode persistent` を使う
